    ...
    */
    ...
    public function wiziapp_getResizedImage($width, $height, $type = 'adaptiveResize', $allow_up = FALSE){
        ...
        if ($this->_cacheExists($cacheFile)){
            ...
            WiziappLog::getInstance()->write('debug', "Before loading image from cache: " . $cacheFile, "image_resizing.getResizedImage");
            $this->handler->load($cacheFile, FALSE);
            WiziappLog::getInstance()->write('debug', "After loading image from cache: " . $cacheFile, "image_resizing.getResizedImage");
        } else {
            $this->imageFile = str_replace(' ', '%20', $this->imageFile);
            WiziappLog::getInstance()->write('debug', "Before resizing image: " . $this->imageFile, "image_resizing.getResizedImage");
            $url = $this->imageFile;
            ...
            WiziappLog::getInstance()->write('debug', "After resizing image: " . $this->imageFile, "image_resizing.getResizedImage");
            WiziappLog::getInstance()->write('debug', "After resizing image URL: " . $url, "image_resizing.getResizedImage");
        }
