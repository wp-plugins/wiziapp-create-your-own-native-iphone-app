...
//    private $default_options = array('duration'=>3600);
    ...
    public function __construct() {
        ...
        if ( function_exists('apc_store') ) {
            WiziappLog::getInstance()->write('info', "APC caching system is active", "WiziappCache.__construct");
            $this->prefix = 'apc';
        }else{
            WiziappLog::getInstance()->write('info', "Did not find APC installed, using file caching", "WiziappCache.__construct");
            $this->prefix = 'file';
            ...
            if(!file_exists($this->fileDir)){
                ...
                if ( $created ){
                    WiziappLog::getInstance()->write('info', "Created the wiziapp file caching directory:".$this->fileDir, "WiziappCache.__construct");
                } else {
                    $this->enabled = FALSE;
                    WiziappLog::getInstance()->write('error', "Couldn't create the wiziapp file caching directory: ".$this->fileDir, "WiziappCache.__construct");
                }
            } else {
                if ( !is_readable($this->fileDir) ){
                    $this->enabled = FALSE;
                    WiziappLog::getInstance()->write('error', "The upload directory exists but its not readable: ".$this->fileDir, "WiziappCache.__construct");
                } else {
                    if ( !is_writable($this->fileDir) ){
                        WiziappLog::getInstance()->write('error', "The upload directory exists but its not writable: ".$this->fileDir, "WiziappCache.__construct");
                        $this->enabled = FALSE;
                    } else {
                        WiziappLog::getInstance()->write('info', "The file cache directory exists and readable and writable: ".$this->path, "WiziappCache.__construct");
                    }
                }
            }
        }
    ...
    public function endCache($output=TRUE){
        ...
        if ( $this->enabled ) {
            ...
            WiziappLog::getInstance()->write('info', "Saving cache key: {$cache_info['key']}",
                                            "WiziappCache.endCache");
            ...
            if ( strlen($content) > 0 ){
                ...
                WiziappLog::getInstance()->write('info', "No content to save in: {$this->prefix}",
                                            "WiziappCache.endCache");
            }
        }
    }
    ...
    private function apcEndCache($key,$content,$options,$headers){
        ...
        WiziappLog::getInstance()->write('info', "Saving cache key: {$key}",
