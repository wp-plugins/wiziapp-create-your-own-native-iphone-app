...
//                    WiziappLog::getInstance()->write('info', ">>> Found iframe::" . print_r($element_val, TRUE), '_process');
    ...
    function _handleVimeoVideo($src){
        ...
        WiziappLog::getInstance()->write('info', ">>> Vimeo API::" . print_r($vimeo_api, TRUE),
            '_handleVimeoVideo');
...
//                $bigThumbElement = $movie['media$group']['media$thumbnail'][$thumbsCount - 1];
    ...
    */
    function _handleFlashObjects($element, $force_src = '', $full_element=array()){
        ...
        WiziappLog::getInstance()->write('info', ">>> The src is {$src}", '_handleFlashObjects');
        if (empty($src)){
            ...
            if (strpos($src, 'vimeo.com') !== FALSE){
                ...
                WiziappLog::getInstance()->write('info', ">>> The obj is {$src}::" . print_r($obj, TRUE), '_handleFlashObjects');
    ...
    function _handleAudio($src, $element, $full_element, $just_return = false) {
        ...
        if ($element) {
            foreach($element['childs'] as $child){
                WiziappLog::getInstance()->write('info', "checking child... : " . print_r($child, TRUE), 'media_extractor.WiziappMediaExtractor._handleAudio');
                // @todo add the thumb;
            }
        }
