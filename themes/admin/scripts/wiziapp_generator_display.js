var WIZIAPP_HANDLER = (function($){
	...
	$(document).ready(function(){
		...
		WIZIAPP_HANDLER.wiziapp_plugins_install = $("#wiziapp_plugins_install");
		...
		$("#wiziapp_main_tabs a").click(showTabIntoIframe);

		// $('#submit_license').bind('click', registerLicense);
	});
	...
	var actions = {
		...
		showPluginsInstall: function(params){
			WIZIAPP_HANDLER.wiziapp_generator_container.hide();

			if ( ! WIZIAPP_HANDLER.plugins_install_loaded ){
				WIZIAPP_HANDLER.plugins_install_loaded = true;

				WIZIAPP_HANDLER.wiziapp_plugins_install
				.children("div:last-child")
				.append('<div style="text-align: center; padding-top: 80px;"><img src="' + WIZIAPP_HANDLER_ADDITIONAL_PARAMS.ajaxLoader + '" alt="ajax loader" /></div>')
				.load( ajaxurl, { action: "wiziapp_plugins_page", } );
			}

			WIZIAPP_HANDLER.wiziapp_plugins_install.show();
		},
	...
	function showTabIntoIframe(event){
		event.preventDefault();

		var request_params = $(this).data('request-params');
		if ( request_params.length === 0 ){
			return false;
		}

		document.getElementById(WIZIAPP_HANDLER_ADDITIONAL_PARAMS.iframeId).contentWindow.postMessage(request_params, "*");

		WIZIAPP_HANDLER.wiziapp_generator_container.show();
		WIZIAPP_HANDLER.wiziapp_plugins_install.hide();
	...
	return {
		...
		wiziapp_plugins_install: {},
		plugins_install_loaded: false
	};
})(jQuery);